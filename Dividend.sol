...
contract Dividend is Ownable {
    ...
    mapping(address => uint256) internal _dividendLast;
    ...
    function dividendAmount(address _uid) public view returns (uint256) {
        if (isContract(_uid)) return 0;
        if (!_OLY.isUser(_uid)) return 0;
        uint256 _totalSupply = _NFT.totalSupply();
        if (_totalSupply == 0) return 0;
        uint256 _balanceOf = _NFT.balanceOf(_uid);
        uint256 _total = _OLY.dividendTotal();
        return
            _total.sub(_dividendLast[_uid]).mul(_balanceOf).div(_totalSupply);
    }

    function dividendTokens() public {
        uint256 _amount = dividendAmount(msg.sender);
        require(_amount > 0);
        require(IBEP20(address(_OLY)).transfer(msg.sender, _amount));
        uint256 _total = _OLY.dividendTotal();
        _dividendLast[msg.sender] = _total;
        _dividendTotal[msg.sender] = _dividendTotal[msg.sender].add(_amount);
    }
    ...
    function dividendTotal(address _uid) public view returns (uint256) {
        return _dividendTotal[_uid];
    }
